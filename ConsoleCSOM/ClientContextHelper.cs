...
{
    public class ClientContextHelper : IDisposable
    {
        ...
        public ClientContext GetContext(Uri web, string userPrincipalName, string userPassword)
        {
            return GetContext(web, userPrincipalName, GetPasswordFromString(userPassword));
        }
        ...
        private static SecureString GetPasswordFromString(string password)
        {
            ...
            foreach (var str in password)
            {
                securePassword.AppendChar(str);
            }
